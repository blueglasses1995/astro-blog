...
export const cvProjects: CVProject[] = [
  {
    ...
    "tasks": [
      ...
      },
      {
        ...
        "decisions": [
          {
            "title": "UseCase/Repository/Domainの3層分離アーキテクチャの採用",
            "detail": "翻訳検証ロジックをUseCase（ビジネスフロー制御）/Repository（データアクセス抽象化）/Domain（ドメインモデル・バリデーション）の3層に分離。生成AIの呼び出しをUseCase層に閉じ込めることで、AIモデル変更時の影響範囲を限定した"
          },
          {
            "title": "生成AIの意思決定をDBに永続化するスキーマ設計",
            "detail": "生成AIが各検証ステップで下した判定（翻訳品質スコア・再翻訳要否・用語修正提案）をすべてDBレコードとして永続化する設計を採用。将来のAIモデル精度比較やプロンプト改善に必要なデータを蓄積する基盤とした"
          }
        ],
        "outcomes": [
          {
            ...
            "metric": "チーム4名での並行開発が可能な設計基盤の確立"
          }
        ],
        "challenges": [
          {
            "title": "生成AIの非決定論的出力をドメインモデルに組み込む設計",
            "resolution": "AIの出力を「判定結果」として型定義し、Domain層でバリデーション後にDBに永続化するフローを設計。AIの出力形式が変わってもDomain層のバリデーションで吸収できる構造にした"
          }
        ]
      },
      {
        ...
        "decisions": [
          {
            "title": "全文検索でも完全一致でもない「準完全一致」方式の設計",
            "detail": "PostgreSQLの全文検索（tsvector）は日本語の助詞・句読点差異で過剰にヒットし、完全一致では表記ゆれで検索漏れが多発。正規化処理（句読点除去・空白統一・助詞パターン許容）を適用した上で文字列比較する中間方式を設計し、精度とリコールを両立させた"
          }
        ],
        "outcomes": [
          {
            ...
            "metric": "用語集検索の精度向上（誤ヒット削減とリコール改善の両立）"
          }
        ],
        "challenges": [
          {
            "title": "日本語テキストの表記ゆれパターンの体系化",
            "resolution": "翻訳対象ドキュメントから頻出する表記ゆれパターン（句読点「、」「，」の混在、助詞「は」「が」の入れ替え、全角半角の混在）を収集・分類。正規化ルールとして実装し、テストケースで網羅的に検証した"
          }
        ]
      },
      {
        ...
        "decisions": [
          {
            "title": "Celery同期ワーカー内でのasyncioイベントループ統合パターンの採用",
            "detail": "Celeryの同期ワーカー（prefork）モデルを維持したまま、タスク内でasyncio.run()によりイベントループを起動するパターンを採用。Celery自体をasyncワーカーに変更する案はエコシステムの互換性リスクが高く、スレッドプール（ThreadPoolExecutor）はIO待ちでスレッドが無駄に占有される問題があったため不採用"
          }
        ],
        "outcomes": [
          {
            ...
            "metric": "外部API呼び出し部分のレイテンシ削減とスループット向上"
          }
        ],
        "challenges": [
          {
            "title": "Celeryの同期実行モデルとasyncioの共存",
            "resolution": "Celeryのpreforkワーカーはプロセスベースのため、各タスク内でasyncioイベントループを新規作成して使い捨てる方式を採用。イベントループのライフサイクルをタスクスコープに限定することで、ワーカー間の干渉を排除した"
          }
        ]
      },
      {
        ...
        "decisions": [
          {
            "title": "段階的マッチング戦略による探索空間の削減",
            "detail": "原文テキストとドキュメント構造（パラグラフ・セル・リスト項目）間のマッチングを、完全一致→正規化一致→部分一致の3段階で順次実行する戦略を採用。上位段階で確定した箇所を探索空間から除外することで、計算量を削減しながら精度を維持した"
          }
        ],
        "outcomes": [
          {
            ...
            "metric": "大規模文書でのマッチング速度改善と精度向上"
          }
        ],
        "challenges": [
          {
            "title": "ドキュメント構造の分割粒度とマッチング精度のトレードオフ",
            "resolution": "Word文書の内部構造（段落・表セル・リスト項目・ヘッダー/フッター）ごとにテキスト分割粒度を調整。分割が細かすぎるとマッチング候補が増えて遅くなり、粗すぎると部分一致の精度が下がる問題を、要素タイプ別の分割ルールで解決した"
          }
        ]
      },
      {
        ...
        "decisions": [
          {
            "title": "Vite + Biomeの選定（webpack + ESLint/Prettierからの脱却）",
            "detail": "既存のwebpackベースのビルド環境をViteに移行し、ESLint+PrettierをBiomeに統合。Viteのホットリロード速度とBiomeの高速lint/formatにより、開発イテレーション速度を大幅に改善した"
          },
          {
            "title": "Volta導入によるNode.jsバージョン管理の統一",
            "detail": "チーム内でNode.jsバージョンの不一致によるビルドエラーが散発していた問題をVoltaで解決。プロジェクトルートにバージョンを固定し、メンバーの環境差異を排除した"
          }
        ],
        "outcomes": [
          {
            ...
            "metric": "テスト・品質管理基盤の確立（ゼロベースからの構築）"
          }
        ],
        "challenges": [
          {
            "title": "既存PHPベースプロジェクトとViteの共存",
            "resolution": "既存のPHP+jQuery環境を壊さず、React部分のみViteで管理するハイブリッド構成を設計。Viteのビルド出力をPHPのテンプレートから読み込む形でインクリメンタルに移行できる構成にした"
          }
        ]
      },
      {
        ...
        "decisions": [
          {
            "title": "OpenAPI仕様をSingle Source of Truthとしたモック自動生成パイプライン",
            "detail": "FastAPIが自動生成するOpenAPI仕様書を唯一の信頼源として、OrvalでTypeScript型定義・APIクライアントを、MSWでモックハンドラーを自動生成するパイプラインを設計。手動でモックを書く方式ではAPIの変更追従が困難なため、仕様書からの自動生成で型安全性とモック鮮度を同時に担保した"
          }
        ],
        "outcomes": [
          {
            ...
            "metric": "フロントエンド・バックエンド間の開発並行性の確立"
          }
        ],
        "challenges": [
          {
            "title": "OpenAPIスキーマとOrval/MSWの型整合性の維持",
            "resolution": "CI上でOpenAPIスキーマからの再生成を自動化し、バックエンドのAPI変更時にフロントエンドの型定義・モックが自動追従する仕組みを構築。型の不整合はTypeScriptのコンパイルエラーとして即座に検出できるようにした"
          }
        ]
      },
      {
        ...
        "decisions": [
          {
            "title": "GHCRプル型デプロイの採用（手動SSH+SCP方式からの移行）",
            "detail": "開発者がSSHでサーバーにログインしてSCPでファイルをアップロードするデプロイ方式から、GHCRにイメージをプッシュし本番サーバーがcronでプルするプル型デプロイに移行。デプロイの再現性を確保し、ロールバックもイメージタグの切り替えで即座に可能にした"
          },
          {
            "title": "GHCRの可視性・権限設定とイメージ管理の設計",
            "detail": "GitHub Container Registryのイメージ可視性をOrganizationレベルで管理し、本番サーバーからのプルに必要な権限設定（Personal Access Token + read:packages scope）を整備。イメージのタグ命名規則も設計した"
          }
        ],
        "outcomes": [
          {
            ...
            "metric": "デプロイの自動化と再現性の確立"
          }
        ],
        "challenges": [
          {
            "title": "cronベースのプルデプロイの信頼性確保",
            "resolution": "プルスクリプトにヘルスチェック・イメージ差分検出・ロールバック機能を組み込み、新イメージのプル失敗時は既存コンテナを維持する設計にした"
          }
        ]
      },
      {
        ...
        "decisions": [
          {
            "title": "スクリプト注入方式によるOnlyOffice設定の外部化",
            "detail": "OnlyOfficeの設定ファイルを直接マウントする方式ではバージョンアップ時に互換性問題が発生するため、Docker起動時にentrypointスクリプトで環境変数から設定ファイルを動的生成する方式を採用。TLS証明書パスとCORSオリジンを環境変数で切り替え可能にした"
          }
        ],
        "outcomes": [
          {
      ...
      },
      {
        ...
        "decisions": [
          {
            "title": "Docker Compose統合による環境構築手順の標準化",
            "detail": "ローカルのReact+Pythonコンテナとリモートサーバー上のPHP+OnlyOfficeを連携させるためのDocker Composeネットワーク設定・環境変数テンプレート・接続確認手順を一つのドキュメントに集約。新規メンバーがドキュメント通りに実行すればE2E環境が再現できる状態を目指した"
          }
        ],
        "outcomes": [
          {
      ...
      },
      {
        ...
        "decisions": [
          {
            "title": "テスト対象の自動化可能領域と手動テスト領域の明確な線引き",
            "detail": "OnlyOfficeエディタのCanvas要素に依存する操作はPlaywrightでの安定的なE2Eテストが困難なため、テスト対象を「翻訳ワークフローの操作フロー・API連携」と「OnlyOffice内のドキュメント操作」に二分し、前者のみE2Eテストでカバーする方針を策定した"
          }
        ],
        "outcomes": [
          {
            ...
            "metric": "E2Eテストによる回帰テストの自動化（テスト可能領域のカバー）"
          }
        ],
        "challenges": [
          {
            "title": "React+Python+OnlyOfficeの3サービス連携テスト環境の構築",
            "resolution": "Docker Composeで3サービスを統合起動し、Playwrightのテストランナーからアクセスできるネットワーク構成を設計。テスト用の初期データ投入とクリーンアップをフィクスチャとして管理し、テストの独立性を確保した"
          }
        ]
      },
      {
        ...
        "decisions": [
          {
            "title": "MCPサーバーによる分散ログのClaude Code統合",
            "detail": "React・Python・Celeryの各コンテナに分散するログをClaude Codeから横断検索できるMCPサーバーを構築。従来はdocker logs + grepで個別にログを確認していたが、MCPツール化によりClaude Codeの会話内からログ検索・フィルタリングが可能になった"
          },
          {
            "title": "agent-browserによるStorybook Story自動生成",
            "detail": "既存のReactコンポーネントからStorybook Storyを自動生成するサブエージェントを構築。agent-browserでコンポーネントの実装を解析し、props・状態パターンを網羅したStoryファイルを自動生成することでUIカタログの整備を加速した"
          }
        ],
        "outcomes": [
          {
            ...
            "metric": "障害調査・開発効率の向上とUIカタログ整備の加速"
          }
        ]
      }
    ]
